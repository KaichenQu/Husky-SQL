...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            # Perform cross attention, input passed through kv matrices to get k, v, pass directly to the right
        elif is_cross_attention:
            # cross_attentions
            key_states = self._shape(self.k_proj(key_value_states), -1, bsz)
            value_states = self._shape(self.v_proj(key_value_states), -1, bsz)
            # Perform cross attention, input encoder's hidden state, apply kv matrices to get k, v
            if prompt:
            ...
            # Function used during generation. Reuse previous k, v, then take hidden state, then do self attention
        else:
            # self_attention
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        prompt = past_prompt
        # TODO: First thing is to add the mask in prompt to the front of attention mask, actually just the last dimension
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        # TODO: Write decoder prompt mask related stuff, cross attention mask and attention mask
        prompt = past_prompt
        if prompt is not None and attention_mask is not None:
