...
def quota_giveup(e):
    """Check if error is quota-related or authentication failure"""
    error_str = str(e).lower()
    return "quota" in error_str or "401" in error_str or "insufficient" in error_str
...
def connect_aiml_gpt(model, prompt, max_tokens, temperature, stop, api_key, reasoning_effort=None):
    """
    Connect to AIML API for GPT-5.1 model

    Args:
        model: Model identifier (e.g., "openai/gpt-5-1")
        prompt: The SQL prompt to complete
    ...
    try:
        ...
        if e.response.status_code == 429:
            raise requests.exceptions.RequestException(f"RateLimitError: {e}")
        return f'error: {e}'
    except Exception as e:
        return f'error: {e}'

def collect_response_from_aiml(db_path_list, question_list, api_key, model, knowledge_list=None, reasoning_effort=None):
    '''
    Modified version of collect_response_from_gpt for AIML API

    :param db_path_list: list of database paths
    :param question_list: list of questions
    :param api_key: AIML API key
    :param model: AIML model identifier
    :param knowledge_list: optional list of external knowledge
    :param reasoning_effort: optional reasoning effort level
    :return: list of SQL responses
    '''
    response_list = []
    ...
    for i, question in tqdm(enumerate(question_list)):
        ...
        # Parse AIML response (Chat Completions format)
        if type(plain_result) == str:
            sql = plain_result
        else:
            # AIML returns: result['choices'][0]['message']['content']
            sql = 'SELECT' + plain_result['choices'][0]['message']['content']

        db_id = db_path_list[i].split('/')[-1].split('.sqlite')[0]
        sql = sql + '\t----- bird -----\t' + db_id # to avoid unpredicted \t appearing in results
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--model', type=str, required=True, default='openai/gpt-5-1')  # Changed from --engine
    args_parser.add_argument('--data_output_path', type=str)
    args_parser.add_argument('--chain_of_thought', type=str)
    args_parser.add_argument('--reasoning_effort', type=str, choices=['low', 'medium', 'high'], default=None)  # NEW parameter
    ...
    if args.use_knowledge == 'True':
        responses = collect_response_from_aiml(
    ...
    print('successfully collect results from {} for {} evaluation; Use knowledge: {}; Use COT: {}; Reasoning effort: {}'.format(
        args.model, args.mode, args.use_knowledge, args.chain_of_thought, args.reasoning_effort
    ))
