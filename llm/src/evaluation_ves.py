...
def compute_ves_by_diff(exec_results,diff_json_path):
    ...
    # Create a mapping of sql_idx to exec_result for partial evaluations
    exec_results_dict = {res['sql_idx']: res for res in exec_results}

    for i,content in enumerate(contents):
        # Skip if we don't have a result for this index
        if i not in exec_results_dict:
            continue

        if content['difficulty'] == 'simple':
            simple_results.append(exec_results_dict[i])
        if content['difficulty'] == 'moderate':
            moderate_results.append(exec_results_dict[i])
        if content['difficulty'] == 'challenging':
            challenging_results.append(exec_results_dict[i])
